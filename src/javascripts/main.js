...
function hideForm(){
    //adds d-none to class list #refers to the id
...
function getCards(){
    if(localStorage.getItem('cards') && localStorage.getItem("cards") != '[]'){//if cards exist and isn't an empty array
    ...
    //if we don't have data for cards or it's empty data we'll use the initial cards
}
...
function addNewCard(event){
    ...
    let cards = getCards()
    if(t && d && p){//if there's actual data
    ...
    document.querySelectorAll('.to-delete').forEach(function(btn){
        btn.onclick = function(event){
            if(confirm("Are you sure you want to delete this card?")){
                cards.splice(event.targert.closest('.col').dataset.ndx, 1)//splice removes items from an array
                localStorage.setItem("cards", JSON.stringify(cards))
                displayCards()
            }
        }
